...
test() {
  ...
  local build_namespace="basic-dotnet-core-build"
  local build_pipline="basic-dotnet-core-pipeline"
  local build_number=$(get_build_number_for "${build_pipline}" "${build_namespace}")

  ## TODO: need to fix this - maybe cancel the first build triggered by applier?
